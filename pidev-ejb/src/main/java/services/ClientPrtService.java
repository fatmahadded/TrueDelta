...
*/
	...
	public Client getClientrByUsername(String username) {
		return (Client) em.createQuery("select c from Client c where c.username=:us").setParameter("us", username).getResultList().get(0);
	}
...
//************************portfolio*******************************************
	...
	public float getGainByClientIdJPQL(int idclient) {
		TypedQuery<Float> query = em.createQuery(
				  "select c.gain from Portfolio c join c.Client e where e.idclient=:idclient", 
				  Float.class);
	...
	public float getRiskByClientIdJPQL(int idclient) {
		TypedQuery<Float> query = em.createQuery(
				  "select c.risk from Portfolio c join c.Client e where e.idclient=:idclient", 
				  Float.class);
				  query.setParameter("idclient", idclient);
				  return query.getSingleResult();	
	}
